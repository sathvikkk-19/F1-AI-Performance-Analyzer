...
class RaceCoordinator:
    ...
    TIER_1_BASE_BONUS = 0.020
    TIER_2_BASE_BONUS = 0.000
    TIER_3_FIRST_HALF_PENALTY = 0.045
    TIER_3_SECOND_HALF_PENALTY = 0.018
    TIER_4_BASE_PENALTY = 0.075
    ...
    DRY_CAR_WEIGHT = 0.72
    DRY_DRIVER_WEIGHT = 0.28

    WET_CAR_WEIGHT = 0.55
    WET_DRIVER_WEIGHT = 0.45
    ...
    def run_race(
        ...
        for entry in grid:
            ...
            # -------------------------------------------------
            # Tier-based race-day opportunity.
            # -------------------------------------------------
            ...
            if tier == 2 and race_random.random() < (0.055 * chaos_factor):
                form -= race_random.uniform(0.04, 0.14)

            elif tier == 3 and race_random.random() < (0.025 * chaos_factor):
                form -= race_random.uniform(0.03, 0.11)

            elif tier == 4 and race_random.random() < (0.010 * chaos_factor):
                form -= race_random.uniform(0.02, 0.08)
    ...
    def _expected_competitive_score(
        ...
        if isinstance(driver_data, dict):
            ...
            try:
                driver_score = float(driver_data)
            except (TypeError, ValueError):
                driver_score = 0.80
        ...
        tier = self._team_tier(team_name)

        if tier == 1:

            tier1_reference = 0.86

            competitive_score = (
                competitive_score * 0.75
                +
                tier1_reference * 0.25
            )

        elif tier == 2:

            competitive_score *= 0.985

        elif tier == 3:

            competitive_score *= 0.965

        elif tier == 4:

            competitive_score *= 0.945
    ...
    def _calibrated_car_score(
        ...
        #
        # The four Tier-1 teams must remain close enough
        # for different circuits to change the order.
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # 2025 historical circuit evidence.
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # Balanced 2026 calibration.
        #
        # Existing car performance remains important,
        # but circuit characteristics now have enough
        # influence to allow Ferrari/Mercedes/Red Bull/
        # McLaren to rotate naturally.
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # Tier-1 circuit variance.
        #
        # Small adjustment only. It must NOT hard-code
        # winners. Circuit demand and team characteristics
        # decide which Tier-1 team benefits.
        # -------------------------------------------------
        ...
        if team_name in self.TOP_WIN_TEAMS:
            ...
            circuit_bias = {
                ...
                "power": {
                    ...
                    "Oracle Red Bull Racing": 0.006,
                    "McLaren Mastercard": 0.004
                },

                "low_drag": {
                    "Mercedes-AMG Petronas": 0.014,
                    "Scuderia Ferrari HP": 0.012,
                ...
                "technical": {
                    "Scuderia Ferrari HP": 0.012,
                    "McLaren Mastercard": 0.010,
                    "Oracle Red Bull Racing": 0.006,
                    "Mercedes-AMG Petronas": 0.004
                },

                "high_speed": {
                    "Oracle Red Bull Racing": 0.012,
                    "McLaren Mastercard": 0.011,
                    "Mercedes-AMG Petronas": 0.009,
                    "Scuderia Ferrari HP": 0.008
                },

                "mixed": {
                    "Oracle Red Bull Racing": 0.009,
                    "McLaren Mastercard": 0.009,
                    "Scuderia Ferrari HP": 0.008,
                    "Mercedes-AMG Petronas": 0.008
                }
            }
